...
from justorservice.service import gamedig, tz
from justorservice.translator import t
from justorservice.version import __version__


class Style(ABC):
    """JustOR Message Style Abstract Class"""
    ...
    def set_footer(self, embed: Embed):
        ...
        #
        today = str(date.today())  # 2025-12-22
        if '-12-22' in today:
        ...
        last_update = datetime.now(tz=tz(self.server.style_data.get('timezone', 'Asia/Jerusalem'))).strftime(time_format)
        ...
        embed.set_footer(text=f'JustOR-StatuService {__version__} | {advertisement} | {last_update}', icon_url=icon_url)
