...
from justorservice.async_utils import run_in_new_loop, to_chunks

from justorservice.environment import AdvertiseType, env
from justorservice.gamedig import GamedigGame
from justorservice.logger import Logger
from justorservice.protocols import Protocol, protocols
from justorservice.server import Server
from justorservice.service import (database, gamedig, invite_link, public,
                                server_limit, timezones, tz, whitelist_guilds)
from justorservice.styles import Style, Styles
from justorservice.translator import Translator, t
from justorservice.version import __version__
...
async def on_guild_join(guild: discord.Guild):
    ...
    # Sync the commands to guild when justor joins a guild.
...
async def on_guild_remove(guild: discord.Guild):
    """Remove all associated servers in database when justor leaves"""
...
def alert_embed(server: Server, alert: Alert):
    ...
    query_time = datetime.now(tz=tz(server.style_data.get('timezone', 'Asia/Jerusalem'))).strftime(time_format)
    query_time = t('embed.alert.footer.query_time', locale).format(query_time=query_time)
    icon_url = 'https://avatars.githubusercontent.com/u/174901607?v=4'
    embed.set_footer(text=f'JustOR-StatuService {__version__} | {query_time}', icon_url=icon_url)
...
async def send_alert(server: Server, alert: Alert):
    ...
    if webhook_url := server.style_data.get('_alert_webhook_url'):
        ...
        username = 'JustOR-Service'
        avatar_url = 'https://avatars.githubusercontent.com/u/174901607?v=4'
